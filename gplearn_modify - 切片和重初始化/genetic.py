...
def _parallel_evolve(n_programs, parents, X, y, sample_weight, seeds, params):
    ...
    for i in range(n_programs):  # n_programs是种群大小
        ...
        program.raw_fitness_ = program.raw_fitness(X, y, curr_sample_weight)  # 计算加权平均值时的权重
    ...
    def fit(self, X, y, sample_weight=None):
        ...
        for gen in range(prior_generations, self.generations):  # 完成剩余迭代
            ...
            if not reinit:
                ...
                reinit = False
                k_optim = int(0.1 * self.population_size)
                population = []
                last_population = deepcopy(self._programs[gen - 1])
                for i in range(max(k_optim, 1)):
                    if self._metric.greater_is_better:
                        population.append(last_population.pop(np.argmax(last_fitness)))
                    else:
                        population.append(last_population.pop(np.argmin(last_fitness)))
                n_jobs, n_programs, starts = _partition_estimators(
                    self.population_size - k_optim, self.n_jobs)  # 少生成k_optim个
                seeds = random_state.randint(MAX_INT, size=self.population_size - k_optim)
                new_population = Parallel(n_jobs=n_jobs,
                                          verbose=int(self.verbose > 1))(
                    delayed(_parallel_evolve)(n_programs[i],
                                              parents,
                                              X,
            ...
            if not self.low_memory:  # 默认是False
                for old_gen in np.arange(gen, 0, -1):  # 创建等差数组，公差是-1
                    indices = []
